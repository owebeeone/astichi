...
| **statement** | block hole + `@astichi_insert` synthesis | [statement/block_hole/](snippets/statement/block_hole/) | [block_hole_generated.py](snippets/statement/block_hole/block_hole_generated.py) |
...
- Use when the example needs **multiple `build()` calls**, **`assign` chains**, or **indexed `Pipeline.Loop.step[i]`** wiring that is awkward to encode as JSON.
