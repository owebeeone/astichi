...
| `astichi.compile` | package users, tests, docs, YIDL generators | `yidl/src/yidl/generation/*.py` | `required-hot` | lower-backed composable facade; Slice 5a stores internal lower template metadata on `BasicComposable` | continue route-through in Slice 5b/6 | compile/final-output goldens plus structural `compile_template_metadata` golden |
