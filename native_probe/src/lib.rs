...
const PARSER_BACKEND: &str = "rustpython-parser 0.4.0";
...
impl LowerComposable {
    ...
    fn __repr__(&self) -> String {
        format!(
            "LowerComposable(filename={:?}, backend={:?}, nodes={})",
            self.filename,
            PARSER_BACKEND,
...
fn parse_module(
    ...
    let filename = filename.unwrap_or_else(|| "<astichi-probe>".to_string());
...
#[pymodule]
fn _native_ast_probe_ext(m: &Bound<'_, PyModule>) -> PyResult<()> {
