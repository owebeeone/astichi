"""F3d: native compile parse replaces CPython ast.parse on the success path."""
...
def test_no_python_parse_capability_when_built() -> None:
    ...
    assert native_no_python_parse_compile_enabled() is True
...
def test_native_compile_skips_python_ast_parse_counter(
    ...
    if load_native_extension(required=False) is None:
        pytest.skip("native engine extension is not built")
    if not native_no_python_parse_compile_enabled():
    ...
    assert counts.get("native_compile_parse", 0) >= 1


def test_native_compile_tree_matches_ast_parse() -> None:
    if load_native_extension(required=False) is None:
        pytest.skip("native engine extension is not built")
    if not native_no_python_parse_compile_enabled():
        pytest.skip("F3d no_python_parse capability not advertised")

    parse_source = "result = astichi_hole(value)\n"
    native_tree = native_compile_tree_from_parse_source(
        parse_source,
        file_name="<oracle>",
    )
    python_tree = ast.parse(parse_source, filename="<oracle>")
    assert ast.dump(native_tree, include_attributes=False) == ast.dump(
        python_tree,
