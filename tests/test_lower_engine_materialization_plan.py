...
def test_scope_lower_pyimport_collision_plan_matches_structural_golden() -> None:
    scope = AssemblyScope(astichi.build())
    ...
    _ACTUAL_STRUCTURAL_DIR.mkdir(parents=True, exist_ok=True)
    actual_path = _ACTUAL_STRUCTURAL_DIR / "scope_pyimport_collision_plan.json"
    actual_path.write_text(actual_text, encoding="utf-8")
    expected_text = (
        _STRUCTURAL_GOLDENS_DIR / "scope_pyimport_collision_plan.json"
    ).read_text(encoding="utf-8")
    assert actual_text == expected_text
